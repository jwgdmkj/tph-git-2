...
# https://github.com/microsoft/Swin-Transformer/blob/main/models/swin_transformer.py, Lline 77
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # First x shape : 1, 64, 512
        # Second x shape : 1, 64, 512
        # Output shape : 36, 64, 512
        B_, N, C = x.shape
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        print('windowAttn output shape ------', x.shape)
        return x
# Above is Same

# Under is Different
class SwinTransformerLayer(nn.Module):  # it is same with original class SwinTransformerBlock(line 175)
    ...
    def __init__(self, dim, num_heads, window_size=8, shift_size=0,
        ...
        # in original, create_mask is in __init__, in '222 line : if self.shift_size > 0:'
        # also, line 245 and 246 added
    ...
    def forward(self, x):
        # reshape x[b c h w] to x[b l c], different with original, batch added

        # First Input : 1, 512, 8, 8
        # Seocond Input : 1, 512, 1, 1
        # output shape : 4, 512, 20, 20
        _, _, H_, W_ = x.shape
        ...
        # --------------------------- original SwinT forward is over ------------------------------@

        x = x.permute(0, 2, 1).contiguous().view(-1, C, H, W)  # b c h w

        if Padding:
            x = x[:, :, :H_, :W_]  # reverse padding
        print('swint shape ------', x.shape)
        return x
...
class SwinTransformerLayer_v2(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0., drop_path=0.,
                act_layer=nn.Mish, norm_layer=nn.LayerNorm, pretrained_window_size=0):
        super().__init__()
        self.dim = dim
...
class SPPFS(nn.Module):
    def __init__(self, c1, c2, n=1):
        ...
        self.cv3 = Conv(c_, c_, 1, 1)
        self.m = nn.ModuleList([nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2) for k in [5, 9, 13]])
        self.cv4 = Conv(4 * c_, c2, 1, 1)
        self.cv5 = Conv(c2, c2, 1, 1)
        self.cv6 = Conv(c2, c2, 3, 1)
        self.cv7 = Conv(c2, c2, 1, 1)
        self.cv8 = Conv(c2, c2, 1, 1)
...
class LePEAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        print('lepe input shape -----', q.shape)
        ### Img2Window
        H = W = self.resolution

        # q.shape : 1, 64, 256
        # because if not last stage, then x1 and x2 split into 2 channels(256, 256)
        B, L, C = q.shape
...
class CSwinBlockLayer(nn.Module):
    '''
    reso : img_size // (2 ** (i+1)), so If reso == split_size == 7, it is last stage.
    '''
...
class CSwinTransformerBlock(nn.Module):
    def __init__(self, c1, c2, num_heads, num_layers, img_size = 224, split_size = [3,5,7]):
        super().__init__()
        self.conv = None
        if c1 != c2:
            self.conv = Conv(c1, c2)
        ...
        # remove input_resolution
        self.blocks = nn.Sequential(*[CSwinBlockLayer(dim=c2, reso = img_size // 2**(i+1), num_heads=num_heads,
...
class C3CSTR(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)    # 512
        self.m = CSwinTransformerBlock(c_, c_, c_//32, n)
